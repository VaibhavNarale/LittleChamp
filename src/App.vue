...
const hideNavigation = computed(() => {
  return route.path === '/login' || route.path === '/register' || route.path === '/forgot-password' || route.path === '/auth/callback'
})

// Add/remove auth-page class to body tag
watch(hideNavigation, (isAuthPage) => {
  if (isAuthPage) {
...
onMounted(async () => {
  ...
  if (authStore.token) {
    ...
    if (previousAuthState && !authStore.authenticated && !hideNavigation.value) {
...
<template>
  <div id="app" :class="{ 'auth-page': hideNavigation }">
    ...
    <template v-if="!hideNavigation">
      ...
      <Header @toggle-side-menu="toggleSideMenu" @toggle-mobile-menu="toggleMobileMenu" />
    </template>
    ...
    <!-- Scroll To Top -->
    <a v-if="!hideNavigation" href="#" class="scrollToTop scroll-btn"><i class="far fa-arrow-up"></i></a>
  </div>
</template>
